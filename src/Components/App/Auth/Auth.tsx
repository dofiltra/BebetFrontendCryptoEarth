...
const Auth: React.FC<IAppProps> = ({showAuth, setLogged, setShowAuth, setProfile}) => {
    ...
    const reg = async () => {
        ...
        let res = await postFormData("/ref_auth/registrate", d)
        if (res) {
            await auth()
        }
    }
            ...
            <div className={'mobile-register'}>
                ...
                <div className={'mobile-register__partner'}>
                    <div className={'mobile-register__partner-title'}>
                        <p>Партнерская программа до <span>60%</span> прибыли</p>
                    </div>
                </div>
                ...
                <div className={'mobile-register__form'} id="register">
                    ...
                    <UiInput onChange={(props) => setTraffic(props)} placeHolder={'Источник трафика'}
                             ...
                             told
                    />
                    <UiInput onChange={(props) => setRefCode(props)} placeHolder={'Промокод'} value={refCode} told/>
                    <UiSelector
                        onSelect={(props) => {
                    ...
                    <UiButton title={'Создать аккаунт'} handleClick={reg}
                              disabled={disabled}
                              width={'100%'}
                    />
                </div>
    ...
    return (
        <div className="register" id="register">
            ...
            <div className={'register-footer'}>
                {footerPoints.map((item, index) => {
                    return (
                        <div className={'register-footer__point'}
                             style={{
                                 backgroundImage: `url(./img/${index}.svg)`
                             }}
                             key={index}
