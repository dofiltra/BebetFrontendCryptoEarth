                ...
                {referent.length > 0 && referent.map((item: any, index: any) => {
                    return (
                        <div key={index} className={'referral__item'}>
                            ...
                            <p>
                                <Button title="История начислений" handleClick={handleEarningsHistory(user.id)} disabled={false} width="150px" transparent={false} black={true}/>
                                <Button title="Информация о выводах" handleClick={handleWithdrawalInfo(user.id)} disabled={false} width="150px" transparent={false} black={true}/>
                                <Button title="Блок/разблок" handleClick={handleBlockUnblockUser(user.id)} disabled={false} width="150px" transparent={false} black={true}/>
                            </p>
                        </div>
                    )
                })}
